...
from procesamiento.utilsformatos import generate_summary_pdf, generate_zonapeso_pdf, generate_zonatotal_pdf
from procesamiento.utilexport import rutaPesodf, listadoTotal, BultosMasivo, Regerospickingmasivo, RegerosSeleccion
from procesamiento.utilsformatoshtml import transformacion_pdf_listado
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                if summary_data:
                    ...
                    # 🛑 DEPURACIÓN: Aplica la función aquí
                    df_depuracion = listadoTotal(df_summary.copy())

                    # Muestra el resultado de la transformación en la interfaz
                    st.header("DEBUG: DataFrame Intermedio")
                    st.dataframe(df_depuracion, use_container_width=True)
                    # 🛑 FIN DEPURACIÓN
